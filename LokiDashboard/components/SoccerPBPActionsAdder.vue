<template>
  <v-row>
    <v-col justify="center">
      <v-card class="text-center mx-10" outlined>
        ...
        <v-tooltip bottom v-for="athlete in uprm_roster" :key="athlete.number">
          ...
          <span v-if="action_buton_pressed === true">{{
            findAthleteName(athlete.athlete_id, uprm_roster)
          }}</span>
    ...
    <v-col allign="center">
      <v-row>
        <v-tooltip bottom v-for="button in action_buttons" :key="button.key">
              ...
              <v-row>
                <v-row allign="center" justify="center">
                  <v-btn
                    ...
                    width="175"
                    v-on="on"
                    >{{ action_info[button.action_type] }}</v-btn
                  >
                  <v-btn
                    class="mx-4"
                    v-else
          ...
          <span>Añadir "{{ action_info[button.action_type] }}"</span>
        </v-tooltip>
      </v-row>
    </v-col>
    <v-col justify="center">
    ...
    <v-dialog v-model="manage_uprm_roster_dialog" max-width="600">
      <v-card>
        <v-card-title class="headline text-center" style="word-break: normal;"
          >Manejo de Atletas UPRM</v-card-title
        >
        <v-card-text class="subtitle-1" style="word-break: normal;"
          >Marque los atletas de UPRM que están participando en este
          evento.</v-card-text
        >
        <v-divider />
        <UPRMSoccerAthlete
        ...
        <v-card-actions fixed>
          ...
          <v-btn
            ...
            text
            @click="manage_uprm_roster_dialog = false"
            >Salir</v-btn
          >
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="manage_opp_roster_dialog" max-width="600">
      <v-card>
        ...
        >
        <v-card-text class="subtitle-1" style="word-break: normal;"
          >Añada, modifique, o remueva los atletas del equipo oponente que
          participan en este evento.</v-card-text
        >
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
            <v-btn
        ...
        <v-card-actions fixed>
          ...
          <v-btn
            color="green darken-1"
            text
            @click="manage_opp_roster_dialog = false"
            >Salir</v-btn
          >
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="add_opp_athlete_dialog" persistent max-width="600px">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="gray darken-3" text @click="close_add_opp_dialog()"
            >Cerrar</v-btn
          >
          <v-btn
...
import UPRMSoccerAthlete from "../components/PBP/UPRMSoccerAthlete";
import OppSoccerAthlete from "../components/PBP/OppSoccerAthlete";
export default {
  ...
  data: () => ({
    ...
    // Action button flags.
    action_buttons: [
      { key: 1, action_type: "Goal", button_state: false },
      { key: 2, action_type: "GoalAttempt", button_state: false },
      { key: 3, action_type: "Assist", button_state: false },
      { key: 4, action_type: "Tackle", button_state: false },
      { key: 5, action_type: "Foul", button_state: false },
      { key: 6, action_type: "YellowCard", button_state: false },
      { key: 7, action_type: "RedCard", button_state: false }
    ],
    action_info: {
      Goal: "Gol",
  ...
  methods: {
    ...mapActions({
      ...
      addPBPAthlete: "soccerPBP/addPBPAthlete",
      notifyNotActionSelected: "soccerPBP/notifyNotActionSelected"
    }),
