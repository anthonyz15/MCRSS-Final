  ...
  <v-card outlined class="pa-md-4 mx-lg-auto">
    <v-row align="center" justify="space-around">
      ...
      <v-col justify="left">
        ...
        <v-row>
          <v-card-text class="subtitle-1 text-left" style="word-break: normal;"
    ...
    <v-dialog v-model="delete_dialog" persistent max-width="300">
      <v-card>
        <v-card-title class="text-center" style="word-break: normal;"
          >Eliminar Atleta Oponente</v-card-title
        >
        <v-card-text
          >Por favor confirme si desea eliminar el atleta oponente
      ...
      <v-card>
        ...
        <v-container>
          <v-row allign="center">
            <v-col>
              <v-card-title
                >Número de Atleta: {{ this.athlete_number }}</v-card-title
              >
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="gray darken-3" text @click="close_edit_dialog()"
            >Cerrar</v-btn
          >
          <v-btn
...
export default {
  ...
  methods: {
    ...mapActions({
      updateOppAthlete: "soccerPBP/updateOppAthlete",
      removeAthlete: "soccerPBP/removeAthlete"
    }),
    async deleteAthlete() {
