...
export default {
  ...
  async handleSetScores({ commit, dispatch }, event_id) {
    try {
      for (let i = 1; i <= 5; i++) {
        ...
        await rtdb()
          .ref("/v1/" + event_id + "/score/set" + i + "-uprm")
        ...
        await rtdb()
          .ref("/v1/" + event_id + "/score/set" + i + "-opponent")
          .on("value", function(snapshot) {
            let entry = { set: i, score: snapshot.val() };
      ...
      dispatch(
        ...
        { text: "Unable to retrieve scores from RTDB.", color: "error" },
        { root: true }
      );
    }
  },
  ...
  async handleCurrentSet({ commit, dispatch }, event_id) {
    try {
      await rtdb()
        .ref("/v1/" + event_id + "/game-metadata/current-set")
        .on("value", function(snapshot) {
      ...
      dispatch(
        "notifications/setSnackbar",
        { text: "Unable to retrieve current set from RTDB.", color: "error" },
        { root: true }
      );
    }
  },
  ...
  async handleUPRMRoster({ commit, dispatch }, event_id) {
    try {
      ...
      dispatch(
        "notifications/setSnackbar",
        {
          text: "Error retrieving UPRM roster update from RTDB.",
          color: "error"
        },
        { root: true }
      );
  ...
  async handleOPPRoster({ commit, dispatch }, event_id) {
    try {
      // Handle roster additions.
      await rtdb()
        .ref("/v1/" + event_id + "/opponent-roster")
        .on("child_added", function(snapshot) {
          let athlete = snapshot.val();
          athlete.key = snapshot.key;
      ...
      dispatch(
        "notifications/setSnackbar",
        {
          text: "Error retrieving opponent roster update from RTDB.",
          color: "error"
        },
        { root: true }
      );
  ...
  async handleGameOver({ commit, dispatch }, event_id) {
    try {
      await rtdb()
        .ref("/v1/" + event_id + "/game-metadata/game-over")
        .on("value", function(snapshot) {
      ...
      dispatch(
        "notifications/setSnackbar",
        { text: "Unable to retrieve game over from RTDB.", color: "error" },
        { root: true }
      );
    }
  },
  ...
  async handleOppColor({ commit, dispatch }, event_id) {
    try {
      await rtdb()
        .ref("/v1/" + event_id + "/game-metadata/opp-color")
        .on("value", function(snapshot) {
      ...
      dispatch(
        "notifications/setSnackbar",
        {
          text: "Unable to retrieve opponent color from RTDB.",
          color: "error"
        },
        { root: true }
      );
  ...
  async handleGameActions({ commit, dispatch }, event_id) {
    try {
      ...
      dispatch(
        "notifications/setSnackbar",
        {
          text: "Error retrieving game actions update from RTDB.",
          color: "error"
        },
        { root: true }
      );
  ...
  async detachUPRMRoster({ commit, dispatch }, event_id) {
    try {
      ...
      dispatch(
        "notifications/setSnackbar",
        {
          text: "Error detaching UPRM roster update from RTDB.",
          color: "error"
        },
        { root: true }
      );
  ...
  async detachOPPRoster({ commit, dispatch }, event_id) {
    try {
      ...
      dispatch(
        "notifications/setSnackbar",
        {
          text: "Error detaching opponent roster update from RTDB.",
          color: "error"
        },
        { root: true }
      );
  ...
  async detachGameOver({ commit, dispatch }, event_id) {
    try {
      ...
      dispatch(
        "notifications/setSnackbar",
        { text: "Unable to detach game over from RTDB.", color: "error" },
        { root: true }
      );
    }
  },
  ...
  async detachOppColor({ commit, dispatch }, event_id) {
    try {
      ...
      dispatch(
        "notifications/setSnackbar",
        { text: "Unable detaching opponent color from RTDB.", color: "error" },
        { root: true }
      );
    }
  },
  ...
  async detachGameActions({ commit, dispatch }, event_id) {
    try {
      ...
      dispatch(
        "notifications/setSnackbar",
        {
          text: "Error detaching game actions updates from RTDB.",
          color: "error"
        },
        { root: true }
      );
  ...
  async getEvent({ commit, dispatch }, event_id) {
    try {
      ...
      if (!!error.response) {
        dispatch(
          "notifications/setSnackbar",
          { text: error.response.data.Error, color: "error" },
          { root: true }
        );
      } else {
        dispatch(
  ...
  async getValidUPRMRoster({ commit, dispatch }, team_id) {
    try {
      const response = await this.$axios.get(
        `/teams/members/public/?team_id=${team_id}`
      );
  ...
  async sendSetAdjust({ commit, dispatch }, payload) {
    try {
      const response = await this.$axios.put(`/pbp/Futbol/set`, payload);
      dispatch(
        "notifications/setSnackbar",
        { text: response.data.MSG, color: "success" },
        { root: true }
      );
    } catch (error) {
      if (!!error.response) {
  ...
  async sendGameAction({ commit, dispatch }, payload) {
    try {
      const response = await this.$axios.post(`/pbp/Futbol/actions`, payload);
      dispatch(
        "notifications/setSnackbar",
        { text: response.data.MSG, color: "success" },
        { root: true }
  ...
  async updateGameAction({ commit, dispatch }, payload) {
    try {
      const response = await this.$axios.put(`/pbp/Futbol/actions`, payload);
      dispatch(
        "notifications/setSnackbar",
        { text: response.data.MSG, color: "success" },
        { root: true }
  ...
  async removeGameAction({ commit, dispatch }, payload) {
    try {
      const response = await this.$axios.delete(
        `/pbp/Futbol/actions?event_id=${payload.event_id}&action_id=${payload.action_id}`
      );
      dispatch(
        "notifications/setSnackbar",
        { text: response.data.MSG, color: "success" },
  ...
  notifyNotActionSelected({ commit, dispatch }) {
    dispatch(
      "notifications/setSnackbar",
      {
        text: "Debe seleccionar el tipo de jugada antes de escoger un jugador.",
        color: "error"
      },
      { root: true }
    );
  ...
  async endPBPSequence({ commit, dispatch }, payload) {
    try {
      const response = await this.$axios.post(`/pbp/Futbol/end`, payload);
      dispatch(
        "notifications/setSnackbar",
        { text: response.data.MSG, color: "success" },
        { root: true }
  ...
  async updateOpponentColor({ commit, dispatch }, payload) {
    try {
      const response = await this.$axios.put(`/pbp/Futbol/color`, payload);
      dispatch(
        "notifications/setSnackbar",
        { text: response.data.MSG, color: "success" },
        { root: true }
  ...
  async addPBPAthlete({ commit, dispatch }, payload) {
    try {
      const response = await this.$axios.post(`/pbp/Futbol/roster`, payload);
      dispatch(
        "notifications/setSnackbar",
        { text: response.data.MSG, color: "success" },
        { root: true }
  ...
  async updateOppAthlete({ commit, dispatch }, payload) {
    try {
      const response = await this.$axios.put(`/pbp/Futbol/roster`, payload);
      dispatch(
        "notifications/setSnackbar",
        { text: response.data.MSG, color: "success" },
        { root: true }
  ...
  async removeAthlete({ commit, dispatch }, args) {
    try {
      const response = await this.$axios.delete(`/pbp/Futbol/roster?${args}`);
      dispatch(
        "notifications/setSnackbar",
        { text: response.data.MSG, color: "success" },
        { root: true }
