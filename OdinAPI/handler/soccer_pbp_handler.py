...
class SoccerPBPHandler:
    ...
    def _handle_pbp_action(self, event_id, action, dao):
        ...
        if len(action) < 2 or len(action) > 4:
            raise Exception(
                "Información de la acción (data) debe tener 2 parámetros para una notificación y 4 para una acción de jugada.")
        ...
        is_valid_athlete = (
                    self._sport_keywords["athlete_prefix"] + str(action["athlete_id"]) in dao.get_uprm_roster(event_id)
                    or (self._sport_keywords["athlete_prefix"] + str(action["athlete_id"])) in dao.get_opponent_roster(
                event_id))
    ...
    def _handle_pbp_edit_action(self, event_id, action_id, new_action, dao):
        ...
        are_same_type = (
                prev_type in self._sport_keywords["scoring_actions"] and new_type in self._sport_keywords[
            "scoring_actions"]
                or prev_type in self._sport_keywords["personal_actions"] and new_type in self._sport_keywords[
                    "personal_actions"]
                or prev_type == "Notification" and new_type == "Notification")

        # Notifications are only posted. No score or set value needs to be modified from a notification.
        if are_same_type and prev_type == self._sport_keywords["notification"]:
            if len(new_action) != 2 or "message" not in new_action or not isinstance(new_action["message"], str) or len(
                    new_action["message"]) < 1 or len(new_action["message"]) > 100:
                raise Exception(
        ...
        if new_team == self._sport_keywords["teams"][0] and (
                self._sport_keywords["athlete_prefix"] + str(int(athlete_id))) not in dao.get_uprm_roster(event_id):
            raise Exception("Atleta de UPRM no existe en el roster.")

        if new_team == self._sport_keywords["teams"][1] and (
                self._sport_keywords["athlete_prefix"] + str(int(athlete_id))) not in dao.get_opponent_roster(event_id):
        ...
        if prev_type in self._sport_keywords["scoring_actions"] and new_type in self._sport_keywords[
            "personal_actions"]:
        ...
        if prev_type in self._sport_keywords["personal_actions"] and new_type in self._sport_keywords[
            "scoring_actions"]:
    ...
    def setOpponentColor(self, event_id, color):
        ...
        try:
            if not isinstance(color, str):
                return jsonify(
                    Error="El color debe estar dado como una secuencia de caracteres que representan un valor HEX (# + 8 valores HEX)."), 400
    ...
    def addOppPlayer(self, event_id, player_info):
        ...
        try:
            ...
            if not isinstance(player_info, dict) or len(
                    player_info) != 2 or not "name" in player_info or not "number" in player_info:
                return jsonify(
                    Error="Información del atleta oponente debe darse en formato JSON y debe contener el nombre y número del atleta."), 400

            if not isinstance(player_info["name"], str) or not isinstance(player_info["number"], int):
                return jsonify(
                    Error="La información del oponente debe darse en el siguiente formato: nombre (secuencia de caracteres) y número (entero)."), 400
            ...
            if opponent_roster and (
                    self._sport_keywords["athlete_prefix"] + str(player_info["number"])) in opponent_roster:
    ...
    def updateOppPlayer(self, event_id, player_info):
        ...
        try:
            ...
            if not opponent_roster or (
                    self._sport_keywords["athlete_prefix"] + str(player_info["number"])) not in opponent_roster:
    ...
    def editPBPAction(self, event_id, action_id, new_action):
        ...
        try:
            ...
            if new_action['action_type']=='Time':
                ...
                self._handle_pbp_edit_action(event_id, action_id, new_action, pbp_dao)
