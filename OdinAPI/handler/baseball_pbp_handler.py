...
from .dao.pbp_dao import PBPDao as BaseballPBPDao
from handler.event import EventHandler
from handler.team import TeamHandler


class BaseballPBPHandler:
    '''
    BaseballPBPHandler - This class handles incomming requests from Odin API's gateway
                 by interacting with the PBPDAO. It is responsible for modifying
                 the state of content stored in the non-relational database,
                 especially information regarding Baseball PBP sequences.
    @author Jose M. Rodriguez Lozano
    '''

    def __init__(self):
        """
        Default constructor - initialize keywords that apply to Baseball PBP sequences.
        """
        ...
        self._sport_keywords = {
            ...
            "uprm-sets": ["/set1-uprm", "/set2-uprm", "/set3-uprm", "/set4-uprm", "/set5-uprm", "/set6-uprm", "/set7-uprm", "/set8-uprm", "/set9-uprm"],
            "opp-sets": ["/set1-opponent", "/set2-opponent", "/set3-opponent", "/set4-opponent", "/set5-opponent", "/set6-opponent", "/set7-opponent", "/set8-opponent", "/set9-opponent"],
            "sport": [
                "Beisbol",
                "Softbol"
            ],
    ...
    def _handle_pbp_action(self, event_id, action, dao):
        ...
        if not "action_type" in action:
            raise Exception(
                "La jugada enviada no está cubierta por el PBP de Beisbol.")
        ...
        if action_type == self._sport_keywords["adjust"]:
            ...
            # Finding score limit based on current set. There is no limit in Baseball
            # limit = 100
            # limit = 25
            # if current_set == 5:
            #     limit = 15
        ...
        raise Exception(
            "La acción indicada no está cubierta por PBP de Beisbol.")
    ...
    def setPBPSequenceOver(self, event_id):
        ...
        try:
            ...
            return jsonify(MSG="Se marcó el partido de Beisbol como finalizado."), 200
